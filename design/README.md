...
└── screens/
    └── <screen-name>/
        └── code.html                 ← per-screen HTML from Stitch's "Get code"
        └── screen.png                ← per-screen Stitch's screenshot
```
...
- **`screens/<screen-name>/code.html`** is **per-screen** — raw output from Stitch's "Get code" button. Untouched. `<screen-name>` is kebab-case and matches the intended Expo Router route.

## Loop

1. Design (or update) screens in Stitch.
2. If the design system changed → re-export `Design.md` and overwrite `design/Design.md`.
3. For each screen → "Get code" → save HTML to `design/screens/<screen-name>/code.html`.
...
- Adding `notes.md` next to a screen's `code.html` is allowed for one-off overrides not expressible in Stitch (e.g. "use the live API endpoint at /api/prayer-times for the data on this screen"). Optional, never required.
